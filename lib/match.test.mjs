...
import { elevationAt, isFree, WORLD_SOLIDS } from './world.mjs';
...
test('botless matches accept humans into empty slots and remove leavers', () => {
  const m = createMatch({ teamSize: 2, fillBots: false });
  assert.equal(m.actors.length, 0);
  assert.equal(joinMatch(m, 'a', 'blue').slot, 0);
  assert.equal(joinMatch(m, 'b', 'blue').slot, 1);
  assert.throws(() => joinMatch(m, 'c', 'blue'));
  leaveMatch(m, 'a');
  assert.equal(m.actors.length, 1);
  assert.equal(joinMatch(m, 'c', 'blue').slot, 0);
  assert.equal(m.actors.filter((a) => a.bot).length, 0);
});
test('competitive mode ends an eliminated round and resets both teams', () => {
  const m = createMatch({ mode: 'competitive' });
  for (const actor of m.actors) actor.bot = false;
  for (const actor of m.actors.filter((a) => a.team === 'red')) {
    actor.health = 0;
    actor.respawn = 4;
  }
  stepMatch(m);
  assert.equal(m.phase, 'round-break');
  assert.equal(m.roundScore.blue, 1);
  advance(m, 3.1);
  assert.equal(m.phase, 'playing');
  assert.equal(m.round, 2);
  assert.ok(m.actors.every((a) => a.health === a.maxHealth));
});
test('capture the flag allows a mounted carrier to steal, score and return the flag on death', () => {
  const m = createMatch({ mode: 'ctf' });
  m.actors.forEach((a) => (a.bot = false));
  const carrier = m.actors.find((a) => a.team === 'blue');
  const redFlag = m.flags.find((f) => f.team === 'red');
  Object.assign(carrier, { x: redFlag.x, z: redFlag.z, protection: 0, mounted: true });
  stepMatch(m);
  assert.equal(redFlag.carrier, carrier.id);
  assert.equal(carrier.flagTeam, 'red');
  carrier.health = 0;
  stepMatch(m);
  assert.equal(redFlag.carrier, '');
  assert.equal(redFlag.x, redFlag.homeX);
  Object.assign(carrier, { health: carrier.maxHealth, x: redFlag.x, z: redFlag.z, protection: 0 });
  stepMatch(m);
  const blueFlag = m.flags.find((f) => f.team === 'blue');
  Object.assign(carrier, { x: blueFlag.homeX, z: blueFlag.homeZ });
  stepMatch(m);
  assert.equal(m.score.blue, 1);
  assert.equal(redFlag.carrier, '');
});
test('capture the flag bots seek the objective in a bot-filled match', () => {
  const m = createMatch({ mode: 'ctf' });
  const blue = m.actors.find((a) => a.team === 'blue');
  const redFlag = m.flags.find((f) => f.team === 'red');
  const before = Math.hypot(blue.x - redFlag.x, blue.z - redFlag.z);
  advance(m, 1);
  assert.ok(Math.hypot(blue.x - redFlag.x, blue.z - redFlag.z) < before);
});
test('archer stairs rise smoothly to mirrored defense platforms', () => {
  assert.equal(elevationAt(20, 14), 0);
  assert.equal(elevationAt(20, 17), 1.5);
  assert.equal(elevationAt(20, 24), 3);
  assert.equal(elevationAt(-20, -17), 1.5);
  assert.equal(elevationAt(-20, -24), 3);
  assert.equal(isFree(20, 19, 0.48, WORLD_SOLIDS), true);
  assert.equal(isFree(15, 24, 0.48, WORLD_SOLIDS), false);
});

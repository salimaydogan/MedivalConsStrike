...
test('arrows travel over time, hit once and do not pass through cover', () => {
  for (const wall of [false, true]) {
    ...
    // The reticle follows the archer's shoulder camera line.
    Object.assign(b, { x: a.x - 0.85, z: wall ? -8 : 10 });
...
test('a bow shot follows both the shoulder aim line and vertical camera aim', () => {
  const m = setup(),
    a = m.actors[0];
  Object.assign(a, { x: 0, z: 0, heading: 0, weapon: 'bow' });
  advance(m, 1, { [a.id]: { weapon: 'bow', attack: true, yaw: 0, pitch: -0.2 } });
  advance(m, 29);
  const arrow = m.projectiles[0];
  assert.ok(arrow);
  assert.ok(Math.abs(arrow.x - 0.85) < 0.001);
...
test('wire commands validate weapon and mount and reject injected horse state', () => {
  ...
  assert.equal(parseCommand({ ...input, pitch: 0.2 }).pitch, 0.2);
  assert.throws(() => parseCommand({ ...input, pitch: 2 }));

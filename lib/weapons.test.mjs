...
test('one shared horse in 2v2, two in 5v5; exclusive occupancy and dismount', () => {
  assert.equal(createMatch().horses.length, 1);
  assert.equal(createMatch({ teamSize: 5 }).horses.length, 2);
  const m = setup(),
    [a, b] = m.actors;
  Object.assign(a, { x: 0, z: 1 });
  Object.assign(b, { x: 0, z: 2 });
  assert.equal(toggleHorse(m, a), true);
  assert.equal(toggleHorse(m, b), false);
  assert.equal(toggleHorse(m, a), true);
  assert.equal(toggleHorse(m, b), true);
  assert.equal(m.horses.length, 1);
});
...
test('arrows travel over time, hit once and do not pass through cover', () => {
  for (const wall of [false, true]) {
    ...
    advance(m, 60);
    assert.equal(b.health, wall ? 100 : 70);
  }
});

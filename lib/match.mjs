...
function fighter(id, team, slot, bot = true, name = '') {
  return {
    ...
    mounted: false,
    horseId: -1,
    weapon: bot ? ['sword', 'spear', 'bow'][slot % 3] : 'sword',
    blocking: false,
    attackTime: -1,
    hitChecked: false,
    regenDelay: 0,
...
export function createMatch(options = {}) {
  ...
  const m = {
    ...
    projectiles:
      /** @type {{id:number,owner:string,team:string,x:number,z:number,vx:number,vz:number,left:number}[]} */ ([]),
    nextProjectile: 0,
    events,
  };
...
function damageActor(m, a, victim, damage, originX = a.x, originZ = a.z) {
  const facing = inArc(
    victim.x,
    victim.z,
    victim.heading,
  ...
  if (victim.blocking && victim.stamina >= 18 && facing) {
    victim.stamina -= 18;
    ...
    if (!a.hitChecked && a.attackTime >= weapon.hit) {
      ...
      if (a.weapon === 'bow') {
        m.projectiles.push({
          ...
          x: a.x,
          z: a.z,
          vx: -Math.sin(a.heading) * 24,
          vz: -Math.cos(a.heading) * 24,
          left: 1.25,
        });
        ...
        if (victim)
          damageActor(
            ...
            weapon.damage +
              (a.mounted
                ? Math.round(Math.min(13, Math.max(0, a.speed)) * 1.4)
                : 0),
          );
      }
    }

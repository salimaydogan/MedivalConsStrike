...
import { WORLD_SOLIDS, isFree, elevationAt } from './world.mjs';
...
export function createMatch(options = {}) {
  ...
  const m = {
    ...
    actors,
    horses: [
      { id: 0, team: 'blue', x: 6, z: 19, homeX: 6, homeZ: 19, homeHeading: Math.PI, heading: Math.PI, rider: '', returnLeft: 0 },
      { id: 1, team: 'red', x: -6, z: -19, homeX: -6, homeZ: -19, homeHeading: 0, heading: 0, rider: '', returnLeft: 0 },
    ],
    // A flag belongs to the named team and can only be carried by its enemy.
    // Returning it on death keeps mounted captures clear and predictable.
    flags: [
      { team: 'blue', x: -6, z: 23, homeX: -6, homeZ: 23, carrier: '' },
      { team: 'red', x: 6, z: -23, homeX: 6, homeZ: -23, carrier: '' },
    ],
    projectiles:
...
export function joinMatch(
  ...
  const usedSlots = new Set(
    m.actors.filter((a) => a.team === team).map((a) => a.slot),
  );
  const slot = index >= 0
    ? m.actors[index].slot
    : Array.from({ length: m.rules.teamSize }, (_, i) => i).find(
        (candidate) => !usedSlots.has(candidate),
      );
  if (slot === undefined) throw Error('Team full');
  const actor = fighter(
    id,
    team,
    slot,
  ...
  applyArenaClass(actor, classId);
  if (index >= 0) m.actors[index] = actor;
  else m.actors.push(actor);
  return actor;
}
export function leaveMatch(m, id) {
  ...
  returnFlag(m, a);
  returnHorse(m, a);
  if (m.rules.fillBots === false) {
    m.actors.splice(index, 1);
    return;
  }
  m.actors[index] = {
    ...a,
...
export function returnFlag(m, a) {
  const flag = m.flags?.find((f) => f.carrier === a.id);
  if (!flag) return;
  flag.carrier = '';
  flag.x = flag.homeX;
  flag.z = flag.homeZ;
  a.flagTeam = '';
  m.events.push({ type: 'flag-return', id: flag.team });
}
...
export function returnHorse(m, a) {
  const horse = releaseHorse(m, a);
  if (horse) horse.returnLeft = 5;
}
...
function damageActor(m, a, victim, damage, originX = a.x, originZ = a.z, projectile = false) {
  ...
  if (victim.health === 0) {
    ...
    returnHorse(m, victim);
    returnFlag(m, victim);
    m.events.push({ type: 'kill', id: victim.id, by: a.id });
  }
}
function updateFlags(m) {
  if (m.rules.mode !== 'ctf') return;
  for (const flag of m.flags) {
    const carrier = m.actors.find((a) => a.id === flag.carrier);
    if (carrier && carrier.health > 0) {
      flag.x = carrier.x;
      flag.z = carrier.z;
      const home = m.flags.find((f) => f.team === carrier.team);
      if (home && Math.hypot(carrier.x - home.homeX, carrier.z - home.homeZ) < 3.2) {
        m.score[carrier.team]++;
        m.events.push({ type: 'flag-score', id: carrier.id, by: flag.team });
        returnFlag(m, carrier);
        if (m.score[carrier.team] >= m.rules.flagLimit) m.phase = 'finished';
      }
      continue;
    }
    if (flag.carrier) {
    ...
    const thief = m.actors.find(
      (a) => a.health > 0 && a.team !== flag.team && !a.flagTeam && Math.hypot(a.x - flag.x, a.z - flag.z) < 2.5,
    );
    if (thief) {
      flag.carrier = thief.id;
      thief.flagTeam = flag.team;
      m.events.push({ type: 'flag-pickup', id: thief.id, by: flag.team });
    }
  }
}
...
function botInput(a, m, dt) {
  ...
  const enemyFlag = m.flags?.find((flag) => flag.team !== a.team);
  const ownFlag = m.flags?.find((flag) => flag.team === a.team);
  const carrying = enemyFlag?.carrier === a.id;
  // In capture the flag, a bot has a clear objective before it chooses a duel:
  // steal the enemy flag, or bring it back to its own base. This also makes
  // bot-filled public rooms playable without waiting for more humans.
  const objective = m.rules.mode === 'ctf' && enemyFlag && ownFlag
    ? carrying
      ? { x: ownFlag.homeX, z: ownFlag.homeZ }
      : !enemyFlag.carrier
        ? { x: enemyFlag.x, z: enemyFlag.z }
        : null
    : null;
  if (!enemy && !objective) return {};
  const target = objective || enemy;
  const distance = Math.hypot(target.x - a.x, target.z - a.z);
  a.navTime -= dt;
  let aim = target;
  if (
    !clearStrike(a.x, a.z, target.x, target.z) ||
    !clearStrike(a.x + 0.6, a.z, target.x + 0.6, target.z)
  ) {
    if (a.navTime <= 0) {
      a.path = findPath(a.x, a.z, target.x, target.z);
      a.navTime = 0.8;
    }
    while (
      a.path.length &&
  ...
  const desired = objective ? 1.6 : a.weapon === 'bow' ? 12 : a.weapon === 'spear' ? 3.4 : 1.9;
  const guard =
    !objective && distance < 3 &&
    enemy.attackTime > 0.1 &&
    enemy.attackTime < 0.32 &&
    Math.floor(m.elapsed + a.slot) % 3 === 0;
  return {
    forward:
      a.path.length || distance > desired ? 1 : a.weapon === 'bow' && distance < 7 ? -1 : 0,
    side: 0,
    yaw,
    sprint: distance > 7,
    guard,
    attack:
      !objective && distance < WEAPONS[a.weapon].range &&
...
export function stepMatch(m, inputs = {}, dt = 1 / 60) {
  ...
  if (m.phase === 'round-break') {
    m.roundBreak = Math.max(0, m.roundBreak - dt);
    if (m.roundBreak === 0) {
      m.round++;
      ...
      Object.assign(m.horses[0], { x: 6, z: 19, heading: Math.PI, rider: '' });
      Object.assign(m.horses[1], { x: -6, z: -19, heading: 0, rider: '' });
      m.projectiles = [];
      m.phase = 'playing';
      m.events.push({ type: 'round-start', id: String(m.round) });
    }
    return m;
  }
  if (m.phase !== 'playing') return m;
  m.elapsed = Math.min(m.rules.duration, m.elapsed + dt);
  for (const horse of m.horses) {
    if (horse.rider || !horse.returnLeft) continue;
    horse.returnLeft = Math.max(0, horse.returnLeft - dt);
    if (horse.returnLeft === 0)
      Object.assign(horse, {
        x: horse.homeX,
        z: horse.homeZ,
        heading: horse.homeHeading,
      });
  }
  ...
  for (const a of actors) {
    ...
    if (a.health <= 0) {
      // Death can be injected by a reconnecting snapshot or test as well as by
      // damageActor; always restore an objective before the respawn moves its
      // former carrier back to base.
      returnFlag(m, a);
      if (m.rules.mode === 'competitive') continue;
    ...
    // A stationary bot must face its target before its next locked swing.
    if (a.bot && !a.mounted && a.attackTime < 0 && Number.isFinite(input.yaw))
      a.heading = input.yaw;
    if (
      input.jump &&
      !a.mounted &&
    ...
    if (
      ...
      a.attackOffset = a.mounted && a.weapon !== 'bow' && Number.isFinite(input.yaw)
        ...
        : 0;
      a.aimPitch = Number.isFinite(input.pitch)
        ? Math.max(-0.4, Math.min(1.25, input.pitch))
        : 0;
      a.hitChecked = false;
      a.stamina -= weapon.cost;
      a.regenDelay = 0.9;
      a.attackCooldown = (a.bot ? 2.1 : 1.1) + a.slot * 0.07;
      a.protection = 0;
    ...
    if (a.bowManual && !a.hitChecked && !a.mounted && Number.isFinite(input.yaw)) {
      motion.heading = input.yaw;
      a.aimPitch = Number.isFinite(input.pitch)
        ? Math.max(-0.4, Math.min(1.25, input.pitch))
        : a.aimPitch;
    }
    ...
    a.elevation = elevationAt(a.x, a.z);
  ...
  for (let i = 0; i < actors.length; i++) {
    ...
    if (!a.hitChecked && a.attackTime >= weapon.hit) {
      ...
      if (a.weapon === 'bow') {
        ...
        const shotPitch = a.aimPitch || 0;
        const speed = 24 * power;
        m.projectiles.push({
          id: ++m.nextProjectile,
          owner: a.id,
          team: a.team,
          // This matches the over-the-shoulder camera target, so the arrow
          // starts on the reticle's line instead of from the actor's centre.
          x: a.x + Math.cos(shotHeading) * 0.85,
          z: a.z - Math.sin(shotHeading) * 0.85,
          y: (a.elevation || 0) + 1.65,
          vx: -Math.sin(shotHeading) * Math.cos(shotPitch) * speed,
          vy: -Math.sin(shotPitch) * speed,
          vz: -Math.cos(shotHeading) * Math.cos(shotPitch) * speed,
        ...
        const victim = actors
          .filter(
            (b) =>
              ...
              Math.abs((a.elevation || 0) - (b.elevation || 0)) < 1.4 &&
    ...
    if (
      ...
      (m.rules.mode === 'tdm' && m.score.red >= m.rules.scoreLimit)
    ) {
      m.phase = 'finished';
      break;
    }
  ...
  if (m.phase === 'playing')
    for (const arrow of m.projectiles) {
      ...
      const contacts = actors
        .filter(
          (a) =>
            ...
            a.health > 0 &&
            a.protection === 0 &&
            Math.abs((arrow.y || 1.35) - ((a.elevation || 0) + 1.1)) < 1.25,
        )
        .map((a) => ({
          a,
          t: segmentContact(
      ...
      if (
        ...
        arrow.left = 0;
      } else if (y <= elevationAt(x, z) + 0.05 || !clearStrike(arrow.x, arrow.z, x, z)) {
        arrow.left = 0;
        m.events.push({
          type: 'impact',
          id: arrow.owner,
  ...
  updateFlags(m);
  if (m.rules.mode === 'competitive' && m.phase === 'playing') {
    const defeated = ['blue', 'red'].find((team) => {
      const members = actors.filter((a) => a.team === team);
      return members.length > 0 && members.every((a) => a.health <= 0);
    });
    if (defeated) {
      const winner = defeated === 'blue' ? 'red' : 'blue';
      m.roundScore[winner]++;
      m.events.push({ type: 'round-end', id: winner });
      if (m.roundScore[winner] >= m.rules.roundLimit) m.phase = 'finished';
      else {
        m.phase = 'round-break';
        m.roundBreak = m.rules.roundBreak;
      }
    }
  }
